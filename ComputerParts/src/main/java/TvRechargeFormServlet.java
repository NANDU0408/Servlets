...
@WebServlet(
        name = "tVRechargeFormServlet",
        urlPatterns = "/tv",
        loadOnStartup = 1
)
public class TvRechargeFormServlet extends HttpServlet {

    public TvRechargeFormServlet() {
        System.out.println("Running TVRechargeFormServlet");
    }

    @Override
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        System.out.println("TVRechargeFormServlet is printing");

        String customerID = request.getParameter("customerID");
        String vendor = request.getParameter("vendor");
        String rechargeAmount = request.getParameter("rechargeAmount");
//        String accept = request.getParameter("term-accept");
//        boolean accept = Boolean.valueOf(request.getParameter("term-accept"));
        ...
        writer.println("<title>TV Recharge Details</title>");
        ...
        writer.println("            <h2 class=\"text-center mb-4 mt-4\">TV Recharge Details</h2>");
        writer.println("            <p><strong>Customer ID:</strong> " + customerID + "</p>");
        writer.println("            <p><strong>Vendor:</strong> " + vendor + "</p>");
        writer.println("            <p><strong>Recharge Amount:</strong> " + rechargeAmount + "</p>");
//        writer.println("<p><strong>Accept:</strong> " + (accept != null && accept.equalsIgnoreCase("true") ? "Yes" : "No") + "</p>");
        writer.println("        </div>");
        writer.println("    </div>");
        writer.println("</div>");
